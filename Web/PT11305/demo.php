...
function get_kh_byid($khid)
{
	...
	$sql = "select * from lop11305 where id ={$khid}";
...
{	
	...
	// câu lệnh truy vấn 
$sql = "INSERT INTO lop11305 (id, name, masv, link, DTB) 
		VALUES 
...
// truy vấn 
	$sql = "
			UPDATE `lop11305` SET `name`='$name',`masv`='$masv',`link`='$link',`DTB`='$DTB' WHERE `id` = '$id' 
			";
// thực hiện câu lệnh truy vấn 
			$query =mysqli_query($conn, $sql);
// trả về 
...
function xoa_sv($id)
{
	...
	$sql = "
		DELETE FROM `lop11305` WHERE `id`='$id'
	" ;
	// thực hiện câu lệnh
	$query = mysqli_query($conn,$sql);
	return $query;
